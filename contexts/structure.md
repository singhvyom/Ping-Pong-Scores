...
- Sheet selection:
  - Opens the spreadsheet by key in `main.py` via `gspread` (`open_by_key`); the actual ID is only in source, not repeated in this doc.
...
The JSON keys are driven by the sheet columns `H` and `V` in `main.py` (with the `V` column mapped to `Vyom` in the returned JSON).
...
`credentials.json` at the repo root holds the Google service account private key used by `main.py`.
